// === functions_esc.ino — ESP32 + THW-1060 (RC ESC) ===
// Hardware: ESPV0.4 (ESP32), THW-1060 ESC (brushed), MPU6050 (HW-123), RS-540, 2S LiPo 2200mAh
...
// Gains (state feedback-like, as in original)
float K1Gain = 175.0f;
float K2Gain = 16.0f;
float K3Gain = 0.04f;
float K4Gain = 13.0f;
...
// Main loop step [ms]
float loop_time = 8.0f;

// Runtime vars
int     pwmCmd = 0;
int32_t motor_speed = 0;
float   motor_speed_enc = 0.0f;

int16_t AcX, AcY, AcXc, AcYc, GyZ, gyroZ;

float angle_prev = 0.0f, velocity = 0.0f;
long  vel_angle_prev_ts = 0;
float vel_angle_prev = 0.0f;
int32_t full_rotations = 0, vel_full_rotations = 0;

// ESP32 ADC range
int max_raw_count = 4095;
int min_raw_count = 0;
...
// ------------ Battery scaling (CALIBRATE THIS!) ------------
// Option A: direct scale: V_batt ≈ analogRead(PIN_VBAT) * VBAT_SCALE
//   Set VBAT_SCALE by measuring with a multimeter and tweaking until it matches.
float VBAT_SCALE = 0.00400f;  // <-- PLACEHOLDER. Calibrate for your divider & ADC.

// Option B: If you know the divider, uncomment and use formula instead.
// const float R_TOP = 100000.0f;  // e.g., 100k from VBAT to ADC
// const float R_BOTTOM = 33000.0f; // e.g., 33k from ADC to GND
...
void escWriteMicroseconds(uint16_t us) {
  const uint32_t period_us = 20000UL;                 // 50 Hz = 20 ms
  const uint32_t maxDuty   = (1UL << ESC_RES_BITS) - 1UL; // 65535
...
void angle_setup() {
  ...
  writeTo(MPU6050, PWR_MGMT_1, 0);                    // wake up
  writeTo(MPU6050, ACCEL_CONFIG, (accSens << 3));     // accel FS
  writeTo(MPU6050, GYRO_CONFIG,  (gyroSens << 3));    // gyro FS
  delay(100);
...
// ------------ Motor control via ESC (map -255..255 to 1000..2000 µs) ------------
void Motor_control(int pwm) {
  // deadband to avoid jitter
  const int dead = 12;
  if (abs(pwm) < dead) pwm = 0;

  // Map to µs around 1500
  int us = 1500 + (int)((float)pwm * (500.0f / 255.0f)); // ±500 µs span
  if (us < 1000) us = 1000;
  if (us > 2000) us = 2000;
